...
class Aggregate(nn.Module):
    ...
    def forward(self, attn, fmap):
        ...
        v = self.to_v(fmap)  # torch.Size([2, 512, 32, 32])
        v = rearrange(v, 'b (h d) x y -> b h (x y) d', h=heads)  # torch.Size([2, 4, 1024, 128])
        out = einsum('b h i j, b h j d -> b h i d', attn, v)  # torch.Size([2, 4, 1024, 128])
