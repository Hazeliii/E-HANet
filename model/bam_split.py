...
from model.bam_split_extractor import BasicEncoder
...
from model.bam import BAM, SpatialGate
from model.bam_split_update import BasicUpdateBlock
...
class ERAFT(nn.Module):
    def __init__(self, args, n_first_channels):
        ...
        self.spatical_att = SpatialGate(gate_channel=hdim)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True):
        ...
        image1.size:  torch.Size([2, 6, 256, 256])
        """
        ...
        with autocast(enabled=self.args.mixed_precision):
            if (self.subtype == 'standard' or self.subtype == 'warm_start'):
                cnet = self.cnet(image2)   # torch.Size([2, 256, 32, 32])
            else:
                raise Exception

            net, inp = torch.split(cnet, [hdim, cdim], dim=1)
            net = torch.tanh(net)
            inp = torch.relu(inp)  # ([2, 128, 32, 32])
            spatial_att = self.spatical_att(inp)
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                net, up_mask, delta_flow = self.update_block(net, inp, corr, flow, spatial_att)
