...
from model.bam_se_extractor import BasicEncoder
...
from model.bam_se_update import BasicUpdateBlock, MyBasicUpdateBlock
...
class ERAFT(nn.Module):
    def __init__(self, args, n_first_channels):
        ...
        self.bam = BAM(gate_channel=self.context_dim)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True):
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                net, up_mask, delta_flow = self.update_block(net, inp, corr, flow, bam_att)
