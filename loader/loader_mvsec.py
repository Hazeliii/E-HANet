...
class MvsecFlow(Dataset):
    def __init__(self, args, filters, type, path):
        ...
        self.dataset = self.get_indices(path, args['datasets'], args['align_to'], args['filter'])
    ...
    def get_indices(self, path, dataset, align_to, filters):
        ...
        for dataset_name in dataset:
            ...
            for subset in dataset[dataset_name]:
                ...
                print('ts pth:', os.path.join(path, dataset_path, ts_path))
                ts = numpy.loadtxt(os.path.join(path, dataset_path, ts_path))
                self.timestamp_files[dataset_name][subset] = ts

                for idx in eval(filters[dataset_name][str(subset)]):
    ...
    def get_data_sample(self, loader_idx):
        ...
        flow_valid[193:,:]=False  # ??? GT中有值，且不为0
        ...
        if input_type == 'events':
            ...
            # print('idx, event_path_new:', idx, event_path_new)
            ...
            if self.evaluation_type == 'sparse':
                ...
                hist = hist.transpose()
                ev_mask = hist > 0
                return_dict['gt_valid_mask'] = torch.from_numpy((flow_valid & ev_mask))
        ...
        # print(ts_old, ts_ev_min, ts_ev_max, ts_new)
