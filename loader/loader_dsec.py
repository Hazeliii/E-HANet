...
class Sequence(Dataset):
    def __init__(self, seq_path: Path, representation_type: RepresentationType, mode: str = 'test',
                 delta_t_ms: int = 100,
                 num_bins: int = 15, transforms=None, name_idx=0, visualize=False, name=''):
        assert num_bins >= 1
        ...
        test_timestamp_file = flow_dir / 'timestamps.txt'
    ...
    def get_data_sample(self, index, crop_window=None, flip=None):
        ...
        output = {'timestamp': self.start_times[index],
                  ...
                  'flow_gt': flow,
                  'valid_2D': valid_2D,
                  'valid_2d_stack': np.stack([valid_2D]*2, axis=0),
                  'valid_2D_stacked': torch.from_numpy(np.stack([valid_2D]*2, axis=0))}
...
class DatasetProvider:
    def __init__(self, dataset_path: Path, representation_type: RepresentationType, delta_t_ms: int = 100, num_bins=15,
        ...
        for child in test_path.iterdir():
            ...
            self.name_mapper_test.append(name)
            print('Sequence name is ', name)
            if type == 'standard':
