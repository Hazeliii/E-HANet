...
class MvsecFlow(Dataset):
    def __init__(self, args, filters, type, path):
        ...
        # data augmentation  transforms.RandomRotation(30),
        self.cropper = transforms.CenterCrop((256, 256))
        self.img_aug = transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.CenterCrop((256, 256))
        ])
        print('The length of train dataset is ', self.__len__())
    ...
    def get_indices(self, path, dataset, align_to, filters):
        ...
        flow_path = '/storage1/dataset/MVSEC/mvsec_45HZ'
        samples = []
    ...
    def get_data_sample(self, loader_idx):
        ...
        flow_dataset = os.path.join('/storage1/dataset/MVSEC/mvsec_45HZ', path_subset)
        ...
        if self.update_rate == 20:
            flow = get_flow_npy(os.path.join(flow_dataset, TEMPLATES.MVSEC_FLOW_GT_FILE.format(idx)))
...
class MvsecFlowRecurrent(Dataset):
    def __init__(self, args, filters, type, path):
        ...
        self.step_size = 1
        self.dataset = MvsecFlow(args, filters, type, path=path)
