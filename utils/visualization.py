...
class DsecFlowVisualizer(BaseVisualizer):
    ...
    def visualize_events(self, image, batch, batch_idx, sequence_name):
        ...
        name_events = TEMPLATES.EVENTS.format('inference_unrectified', int(batch['file_index'][batch_idx].item()))
    ...
    def __call__(self, batch, batch_idx, epoch=None):
        for batch_idx in range(len(batch['file_index'])):
            ...
            if batch['visualize'][batch_idx]:
                ...
                flow_est = batch['flow_est'][batch_idx].clone()
                valid_stack = batch['valid_2d_stack'][batch_idx]
                mask = (valid_stack == 0)
                flow_est[mask] = 0.0
                self.visualize_flow_colours(flow_est, batch['file_index'][batch_idx],
                                            dataset_name='dsec',
                                            epoch=epoch,
                                            is_gt=False,
                                            fix_scaling=None,
                ...
                flow_gt[~batch['valid_2D_stacked'][batch_idx].bool()] = 0.0
                self.visualize_flow_colours(flow_gt, batch['file_index'][batch_idx],
                                            dataset_name='dsec',
                                            epoch=epoch,
                                            is_gt=True,
                                            fix_scaling=None,
